 ...
 *  - election_participation: (election_id, student_id, ballot_id) — proves
 *    WHO voted, without any of their choices.
 *  - ballots / ballot_items: keyed by the random ballot_id, listing choices
...
votingRouter.get('/status', (req, res) => {
  ...
  db.prepare(`UPDATE elections SET status = 'OPEN' WHERE status = 'SCHEDULED' AND opens_at <= ?`).run(nowIso);
  db.prepare(`UPDATE elections SET status = 'CLOSED' WHERE status = 'OPEN' AND closes_at <= ?`).run(nowIso);

  const open = openElectionNow();
  const all = db.prepare(`SELECT id, name, slug, description, opens_at, closes_at, status, results_published_at FROM elections WHERE status != 'DRAFT' ORDER BY opens_at DESC LIMIT 10`).all();
  ...
  const already = db.prepare(`SELECT id, confirmation_code, voted_at FROM election_participation WHERE election_id = ? AND student_id = ?`).get(open.id, student.id) as any;
  ...
  const positions = (db.prepare(`SELECT * FROM election_positions WHERE election_id = ? AND active = 1 ORDER BY display_order, id`).all(open.id) as any[]).map((p) => ({
    ...p,
    contestants: db.prepare(`SELECT id, full_name, level, photo_filename, manifesto FROM contestants WHERE position_id = ? AND active = 1 ORDER BY id`).all(p.id),
  }));
 ...
 */
votingRouter.post('/submit', (req, res) => {
  ...
  db.prepare(`UPDATE elections SET status = 'OPEN' WHERE status = 'SCHEDULED' AND opens_at <= ?`).run(nowIso);
  db.prepare(`UPDATE elections SET status = 'CLOSED' WHERE status = 'OPEN' AND closes_at <= ?`).run(nowIso);
  const election = openElectionNow();
  if (!election) {
    return res.status(403).json({ error: 'There is no election open right now.' });
  }

  // 3. Has this student already voted in this election?
  const existingParticipation = db.prepare(`SELECT id FROM election_participation WHERE election_id = ? AND student_id = ?`).get(election.id, student.id);
  ...
  const positions = db.prepare(`SELECT id, name FROM election_positions WHERE election_id = ? AND active = 1 ORDER BY display_order, id`).all(election.id) as any[];
  const positionIds = positions.map((p) => p.id);
  const contestants = db.prepare(`SELECT id, position_id FROM contestants WHERE active = 1`).all() as any[];
  ...
  const tx = db.transaction(() => {
    db.prepare(`INSERT INTO election_participation (election_id, student_id, confirmation_code) VALUES (?, ?, ?)`).run(
      election.id,
      student.id,
      confirmationCode,
    );
    const ballotInfo = db.prepare(`INSERT INTO ballots (election_id) VALUES (?)`).run(election.id);
    const ballotPk = Number(ballotInfo.lastInsertRowid);
    const insertItem = db.prepare(`INSERT INTO ballot_items (ballot_id, position_id, contestant_id) VALUES (?, ?, ?)`);
    for (const [rawPid, rawCid] of entries) {
      insertItem.run(ballotPk, Number(rawPid), Number(rawCid));
    }
  });
