...
// ---------------------------------------------------------------------------
studentAuthRouter.post('/login', loginLimiter, (req, res) => {
  ...
  const student = db
    .prepare(`SELECT id, full_name, matric_number, email, level, status, password_hash, must_change_password FROM students WHERE matric_number = ?`)
    .get(normalized) as
  ...
  if (!student || !student.password_hash || !bcrypt.compareSync(String(password), student.password_hash)) {
    audit({ actorType: 'student', action: 'student_login_failed', description: `Failed login attempt for ${normalized}` });
  ...
  audit({ actorType: 'student', actorId: student.id, actorLabel: student.matric_number, action: 'student_login', description: 'Student signed in' });
  res.json({
    student: {
      id: student.id,
...
// ---------------------------------------------------------------------------
studentAuthRouter.post('/change-password', requireStudent, (req, res) => {
  ...
  const row = db.prepare(`SELECT must_change_password FROM students WHERE id = ?`).get(s.id) as { must_change_password: number };
  if (!row.must_change_password) {
    ...
    const cur = db.prepare(`SELECT password_hash FROM students WHERE id = ?`).get(s.id) as { password_hash: string };
  ...
  db.prepare(`UPDATE students SET password_hash = ?, must_change_password = 0, invite_token_hash = NULL, invite_expires_at = NULL, updated_at = datetime('now') WHERE id = ?`).run(hash, s.id);
  audit({ actorType: 'student', actorId: s.id, actorLabel: s.matric_number, action: 'password_set', description: 'Student set a new password' });
  res.json({ ok: true });
});
...
// ---------------------------------------------------------------------------
studentAuthRouter.get('/me', requireStudent, (req, res) => {
  const s = (req as any).student as { id: number };
  const student = db
    .prepare(`SELECT id, full_name, matric_number, email, level, status, source, must_change_password, created_at FROM students WHERE id = ?`)
    .get(s.id);
  res.json({
    student,
