...
adminAuthRouter.post('/login', loginLimiter, (req, res) => {
  const { email, password } = req.body ?? {};
  if (!email || !password) {
    return res.status(400).json({ error: 'Email and password are required.' });
  }
  const admin = db
    .prepare(`SELECT id, name, email, password_hash, role, must_change_password FROM admins WHERE lower(email) = lower(?)`)
  ...
  if (!admin || !bcrypt.compareSync(String(password), admin.password_hash)) {
    audit({ actorType: 'system', action: 'admin_login_failed', description: `Failed admin login for ${String(email).trim()}` });
  ...
  audit({ actorType: 'admin', actorId: admin.id, actorLabel: admin.name, action: 'admin_login', description: 'Admin signed in' });
...
adminAuthRouter.post('/logout', (req, res) => {
  ...
  if (admin) audit({ actorType: 'admin', actorId: admin.id, actorLabel: admin.name, action: 'admin_logout', description: 'Admin signed out' });
...
adminAuthRouter.get('/me', requireAdmin, (req, res) => {
  ...
  const admin = db.prepare(`SELECT id, name, email, role, must_change_password, last_login_at FROM admins WHERE id = ?`).get(me.id);
  res.json({ admin });
});

adminAuthRouter.post('/change-password', requireAdmin, (req, res) => {
  const admin = (req as any).admin as { id: number; name: string };
  const { currentPassword, newPassword } = req.body ?? {};
  if (!newPassword) return res.status(400).json({ error: 'New password is required.' });
  if (String(newPassword).length < 8) return res.status(400).json({ error: 'New password must be at least 8 characters.' });

  const row = db.prepare(`SELECT password_hash, must_change_password FROM admins WHERE id = ?`).get(admin.id) as {
  ...
  audit({ actorType: 'admin', actorId: admin.id, actorLabel: admin.name, action: 'admin_password_change', description: 'Admin changed own password' });
  res.json({ ok: true });
});
