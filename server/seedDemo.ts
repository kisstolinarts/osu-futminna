...
import { prepare, tx, migrate } from './dbCloud';
 ...
 * (Point DATABASE_URL/TURSO_AUTH_TOKEN at a cloud DB to seed that instead.)
 */
...
async function main() {
  ...
  const wipeTables = [
    'ballot_items', 'ballots', 'election_participation', 'contestants',
    'election_positions', 'elections', 'approved_whatsapp_numbers', 'students',
    'audit_logs', 'app_settings', 'admins',
  ];
  await tx(async (t) => {
    for (const name of wipeTables) await t.execute({ sql: `DELETE FROM ${name}` });
  });
  ...
  const insertAdmin = prepare(`INSERT INTO admins (name, email, password_hash, role, must_change_password) VALUES (?, ?, ?, ?, ?)`);
  for (const a of admins) await insertAdmin.run(a.name, a.email, hash(a.pw), a.role, a.role === 'SUPER_ADMIN' ? 0 : 1);
  ...
  const insertApproved = prepare(`INSERT OR IGNORE INTO approved_whatsapp_numbers (phone_raw, phone_normalized, note) VALUES (?, ?, 'demo seed')`);
  for (const s of students) {
    const norm = normalizePhone(s.phone);
    const local = toLocalMobile(s.phone);
    await insertStudent.run(s.name, s.matric, s.email, s.phone, norm, s.level, local ? hash(local) : null);
    if (norm) await insertApproved.run(s.phone, norm);
  }

  // One OPEN election
  const opens = isoDaysFromNow(-1);
  const closes = isoDaysFromNow(5);
  const electionInfo = await prepare(`INSERT INTO elections (name, slug, description, opens_at, closes_at, status) VALUES (?, ?, ?, ?, ?, 'OPEN')`).run(
  ...
  const posIds: number[] = [];
  const insertPos = prepare(`INSERT INTO election_positions (election_id, name, description, display_order) VALUES (?, ?, ?, ?)`);
  for (const p of positions) {
    const info = await insertPos.run(electionId, p.name, p.desc, p.order);
    posIds.push(Number(info.lastInsertRowid));
  }
  ...
  const insertCont = prepare(`INSERT INTO contestants (position_id, full_name, level, manifesto) VALUES (?, ?, ?, ?)`);
  for (const c of candidates) await insertCont.run(c.pos, c.name, c.level, c.manifesto);
